...
import io.github.mosser.arduinoml.kernel.behavioral.LogicalTransition;
...
public class ToWiring extends Visitor<StringBuffer> {
    ...
    @Override
    public void visit(LogicalTransition transition) {
        if (context.get("pass") == PASS.ONE) {
            return;
        }
        if (context.get("pass") == PASS.TWO) {
            // Compile the logical expression to C++ code
            String condition = compileLogicalExpression(transition.getExpression());

            // Generate the if statement with the condition
    ...
    private String compileLogicalExpression(LogicalExpression expr) {
        ...
        if (expr instanceof BinaryExpression) {
            ...
            return "(" + left + " " + bin.getOperator().getOperator() + " " + right + ")";
        }
